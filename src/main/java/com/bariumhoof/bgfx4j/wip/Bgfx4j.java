...
//    public static int U_INT16_MAX = 65535;
    ...
    public static void vertexPack(float[] input, boolean isNormalized, @NotNull BGFX_ATTRIB attrib, @NotNull VertexLayoutOld layout, int index) {
        bgfx_vertex_pack(input, isNormalized, attrib.VALUE, layout.get(), ByteBuffer.allocate(input.length), index);
    }

    public static void vertexPack(@NotNull FloatBuffer input, boolean isNormalized, @NotNull BGFX_ATTRIB attrib, @NotNull VertexLayoutOld layout, int index) {
        bgfx_vertex_pack(input, isNormalized, attrib.VALUE, layout.get(), ByteBuffer.allocate(input.capacity()), index);
    }

    public static void vertexConvert(@NotNull ByteBuffer source, @NotNull VertexLayoutOld sourceLayout, @NotNull VertexLayoutOld dstLayout, int num) {
    ...
    public static void weldVertices(short[] output, @NotNull VertexLayoutOld layout, boolean is32BitData, @NotNull ByteBuffer data, float epsilon) {
        bgfx_weld_vertices(output, layout.get(), data, is32BitData, epsilon);
    }
